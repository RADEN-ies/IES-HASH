...
info = '\033[1;32m[*]\033[1;m'
que =  '\033[1;32m[*]\033[1;m'
bad = '\033[1;31m[*]\033[1;m'
good = '\033[1;32m[*]\033[1;m'
run = '\033[1;32m[>]\033[1;m'
...
/___/_____//____/     /_/ /_/_/  |_/____/   v.1.0
        ...
        if match:
            ...
            if match:
                ...
                if len(purl) > 0:
                    ...
                    if not args.path:
                        print "%s TYPE HASH TIDAK DI TEMUKAN " % bad
    elif len(hashvalue) == 40:
        if not args.path:
            print "%s HASH : SHA1" % info
        beta()
    elif len(hashvalue) == 64:
        if not args.path:
            print "%s HASH : SHA-256" % info
        Lambda()
    else:
        print "%s error hash tidak di temukan " % bad
...
if args.path:
    ...
    hashvalue = raw_input('%s HASH : ' % que).lower()
    crack(hashvalue)
